  ...
  namespace config_server {
    ...
    void tair_config_server::start()
    {
      ...
      if(ret) {
		...
		//×¢²á¼àÌıÊÂ¼ş£¬½«Êı¾İ´¦ÀíÀàpacket_streamer ×Ô¶¯»Øµ÷´¦Àí¶ÁĞ´Êı¾İ
		//Êµ¼ÊÔÚÕâÀïÒÑ¾­½¨Á¢ÁËÒ»¸ösocket·şÎñÆ÷£¬µÈ´ıepollÑ­»·¿ªÊ¼
		//´«Èëtair_config_server ´¦ÀíÊı¾İtair_config_server ÓĞpaketqueue¶ÔÏó£¬ÓÃÓÚ·ÅÈëÊı¾İºÍÈ¡³öÊı¾İ
      ...
      if(ret) {
        ...
        packet_transport.start();//Æô¶¯epollµÄÑ­»·
        heartbeat_transport.start();//Æô¶¯epollµÄÑ­»·
        
        //²Â²â¿ÉÄÜÊÇÔÚÊµÏÖÅäÖÃĞÅÏ¢µÄ¶ÁÈ¡Ïà¹Ø
    ...
    int tair_config_server::initialize()
    {
      ...
      //Ä¬ÈÏµÄ2¸öconfig_server² ¼ì²é±¾»úÊÇ·ñÔÚconfig_serverµÄÅäÖÃÎÄ¼şÖĞ
	  uint64_t id;
      int server_index = 0;
	  ...
	  //´´½¨Ïß³Ì³Ø£¬ÉèÖÃÏß³Ì³ØµÄÏß³ÌÊıÁ¿
	  //this ¼°tair_config_server¼Ì³ĞÁËIPacketQueueHandler ÊµÏÖÁË¾ßÌå´¦ÀíÊı¾İµÄ·½·¨
	  //½¨Á¢Ò»¸öqueue´æ·Å½ÓÊÕµÄpaket,²¢ÇÒ²»¶ÏµÄÈ¡³öÀ´µ÷ÓÃIPacketQueueHandler ´¦Àí·½·¨´¦ÀíÊı¾İ
