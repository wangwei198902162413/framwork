...
namespace tair
{
  ...
  enum
  {
    ...
    TAIR_REQ_SET_NS_ATTR_PACKET = 2166,

    TAIR_REQ_GET_NS_ATTR_PACKET = 2167
  };
...
#define PUT_CSTR_TO_BUFFER(output,buff,buff_len) do {               \
    (output)->writeInt32((buff_len));                               \
    (output)->writeBytes((buff),(buff_len));                        \
}while(0)
...
#define GET_CSTR_FROM_BUFFER(input,buff,buff_len) do {              \
    if((input)->getDataLen() < 4) {                                 \
        log_warn("buffer data too few.");                           \
        return false;                                               \
    }                                                               \
    (buff_len) = (input)->readInt32();                              \
    if ((buff_len) < 0 || (input)->getDataLen() < (buff_len)) {     \
        log_warn("buffer data too few.");                           \
        return false;                                               \
    }                                                               \
    if ((buff_len) == 0) {                                          \
        ...
        if ((input)->readBytes(buff, buff_len) == false) {          \
            log_warn("buffer data too few.");                       \
            return false;                                           \
        }                                                           \
    }                                                               \
