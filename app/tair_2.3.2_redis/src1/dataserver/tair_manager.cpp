...
#define STAT_KEY_VERIFY  do {                                           \
   ...
   int tair_manager::info (data_entry & info)
   {
      ...
      snprintf(buffer, 1024, "tair 2.3.5.1\r\n"
                             "used_cpu_sys: %f\r\n"
                             "used_cpu_user: %f\r\n"
                             "swap used %ld\r\n",
   ...
   int tair_manager::lazyclear(int area, const char* password, const int pwd_len)
   {
      if (status != STATUS_CAN_WORK) {
         return TAIR_RETURN_SERVER_CAN_NOT_WORK;
      }

      return storage_mgr->lazyclear(area, password, pwd_len);
   }
   ...
   int tair_manager::setnsattr(int area, const char* attr, const int attr_len,
           const char* val, const int val_len)
   {
      ...
      return storage_mgr->setnsattr(area, attr, attr_len, val, val_len);
   }

   int tair_manager::getnsattr(int area, const char* attr, const int attr_len,
           char** val, int* val_len)
   {
      ...
      return storage_mgr->getnsattr(area, attr, attr_len, val, val_len);
   }
