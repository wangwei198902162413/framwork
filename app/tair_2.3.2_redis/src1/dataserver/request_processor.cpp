...
namespace tair {
   ...
   int request_processor::process (request_lazy_remove_area * request, bool & send_return)
   {
     ...
     rc = tair_mgr->lazyclear(request->area, request->password, request->pwd_len);
   ...
   int request_processor::process (request_set_ns_attr * request, bool & send_return)
   {
     PROC_BEFORE(response_simple_packet);
     rc = tair_mgr->setnsattr(request->area, request->attr, request->attr_len,
             request->value, request->value_len);
     return PROC_AFTER;
   }

   int request_processor::process (request_get_ns_attr * request, bool & send_return)
   {
     PROC_BEFORE(response_simple_packet);
     rc = tair_mgr->getnsattr(request->area, request->attr, request->attr_len,
             &(resp->buff), &(resp->buff_len));
     return PROC_AFTER;
   }
