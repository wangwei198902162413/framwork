...
/*common*/
    ...
    virtual int setnsattr(uint32_t area, const char* attr, const int attr_len,
            const char* val, const int val_len, const char* password, const int len)
    {
        int ret = verify_password(area, password, len);
        if (ret == REDIS_VERIFY_FAILED) {
            return REDIS_VERIFY_FAILED;
        }
        /* Notes: it operator area, not unit, so it should lock all unit */
        redisAreaConf* area_conf = context->get_area_conf(area);
        pthread_mutex_lock(&(area_conf->lock));
        ret = session->setdbattr(area, attr, attr_len, val, val_len);
        pthread_mutex_unlock(&(area_conf->lock));
        return ret;
    }

    virtual int getnsattr(uint32_t area, const char* attr, const int attr_len,
            char** val, int* val_len, const char* password, const int len)
    {
        int ret = verify_password(area, password, len);
        if (ret == REDIS_VERIFY_FAILED) {
            return REDIS_VERIFY_FAILED;
        ...
        redisAreaConf* area_conf = context->get_area_conf(area);
        pthread_mutex_lock(&(area_conf->lock));
        ret = session->getdbattr(area, attr, attr_len, val, val_len);
        pthread_mutex_unlock(&(area_conf->lock));
        return ret;
    }
    ...
    virtual int verify_password(uint32_t area, const char* password, const int len)
    {
        /* Notes: lock area */
        redisAreaConf* area_conf = context->get_area_conf(area);
        pthread_mutex_lock(&(area_conf->lock));
        int ret = session->verify_password(area, password, len);
        pthread_mutex_unlock(&(area_conf->lock));
        return ret;
    }

    virtual int lazyflushdb(uint32_t area, const char* password, const int len) {
        int ret = verify_password(area, password, len);
        if (ret == REDIS_VERIFY_FAILED) {
            return ret;
        }
...
/*string*/
	/**
     * [put item data into redis_db]
     * @param  MKEY  [area 数据在的数据的索引]
     * @param  MDETA [包括一些校验信息]
     * @param  item  [要保存的数据]
     * @param  EORN  [待观察]
     * @return       [description]
     */
