...
{
    redisDb *db = context.server->db + area;
    if (db->status & REDIS_DB_STATUS_LOADING) {
        return REDIS_IS_LOADING;
    }
    if (db->status & REDIS_DB_STATUS_DUMPING) {
        return REDIS_OK;
    }
...
int redis_db_session::verify_password(uint32_t area, const char* pwd, const int pwd_len)
{
    char* area_pwd = context.server->area_confs[area].password;
    if (area_pwd == NULL && pwd == NULL && pwd_len == 0) {
        return REDIS_OK;
    }
    if (area_pwd == NULL && strcmp(context.server->password, pwd) == 0) {
        return REDIS_OK;
    }
    if (strcmp(area_pwd, pwd) == 0) {
        return REDIS_OK;
    }
    return REDIS_VERIFY_FAILED;
}

int redis_db_session::setdbattr(uint32_t area, const char* attr, int attr_len,
        const char* val, int val_len)
{
    if (!strcmp(RDB_AREA_PASSWORD, attr)) {
        /* set password */
        if (val == NULL || val_len <= 0) {
            return REDIS_ERR_ATTR;
        }
        char* pwd = (char*)zmalloc_simple(sizeof(char) * (val_len + 1));
        if (pwd == NULL) {
            return REDIS_ERR_MEMORY_ALLOCATE_ERROR;
        }
        strcpy(pwd, val);
        if (context.server->area_confs[area].password != NULL) {
            zfree_simple(context.server->area_confs[area].password);
        }
        context.server->area_confs[area].password = pwd;
        return REDIS_OK;
    }

    long size = strtol(val, NULL, 10);
    if (size <= 0) {
        return REDIS_ERR_ATTR;
    }

    if (!memcmp(RDB_LIST_MAX_SIZE, attr, 13)) {
        context.server->area_confs[area].list_max_size = size;
    } else if (!memcmp(RDB_HASH_MAX_SIZE, attr, 13)) {
        context.server->area_confs[area].hash_max_size = size;
    } else if (!memcmp(RDB_ZSET_MAX_SIZE, attr, 13)) {
        context.server->area_confs[area].zset_max_size = size;
    } else if (!memcmp(RDB_SET_MAX_SIZE, attr, 12)) {
        context.server->area_confs[area].set_max_size = size;
    } else {
        return REDIS_ERR_ATTR;
    }
    return REDIS_OK;
...
int redis_db_session::getdbattr(uint32_t area, const char* attr, int attr_len,
        char** val, int* val_len)
{
    if (!strcmp(RDB_AREA_PASSWORD, attr)) {
        /* get password */
        char* pwd = context.server->password;
        if (context.server->area_confs[area].password != NULL) {
            pwd = context.server->area_confs[area].password;
        }
        int len = strlen(pwd);
        if (len <= 0) {
            return REDIS_ERR_ATTR;
    ...
    if (!memcmp(RDB_LIST_MAX_SIZE, attr, 13)) {
        snprintf(pval, 32, "%d", context.server->area_confs[area].list_max_size);
    } else if (!memcmp(RDB_HASH_MAX_SIZE, attr, 13)) {
        snprintf(pval, 32, "%d", context.server->area_confs[area].hash_max_size);
    } else if (!memcmp(RDB_ZSET_MAX_SIZE, attr, 13)) {
        snprintf(pval, 32, "%d", context.server->area_confs[area].zset_max_size);
    } else if (!memcmp(RDB_SET_MAX_SIZE, attr, 12)) {
        snprintf(pval, 32, "%d", context.server->area_confs[area].set_max_size);
    } else {
        delete []pval;
        *val = NULL;
        *val_len = 0;
...
int redis_db_session::set(MKEY, MDETA, value_item* item, EORN)
{
    int ret;
    redisClient *client;
	//加锁，在context 变量中有rdb server ,定义了锁
    DB_CONTEXT_LOCK(area);
	//#define SETNX_COMMAND 2
	//getCommand(SETNX_COMMAND)->argc setnx命令的参数的个数
...
int redis_db_session::lrpop(MKEY, MDETA, int count, LORR, OITEMSVSN)
{
    ...
    *item = new value_item[*item_num];
    ...
    {
        if(node->type == NODE_TYPE_ROBJ) {
            robj* obj = getDecodedObject((robj*)(node->obj.obj));
    ...
    *item = new value_item[*item_num];
    ...
    while((node = nextValueItemNode(&it)) != NULL)
    {
        if(node->type == NODE_TYPE_ROBJ) {
            ...
            char buf[32];
            ll2string(buf, 32, node->obj.llnum);
            (*item)[now].data_len = strlen(buf);
            (*item)[now].data = new char[(*item)[now].data_len];
            memcpy((*item)[now].data, buf, (*item)[now].data_len);
        }
        ++now;
    }
    freeValueItemIterator(&it);
            ...
            size_t value_in_list_len, value_item* value_insert)
{
    ...
    redisClient *client;
    DB_CONTEXT_LOCK(area);
    ret = get_redis_client(&client,area,getCommand(LINSERT_COMMAND)->argc);
    if (ret != REDIS_OK) {
        return ret;
    }
...
int redis_db_session::filter_field(redisServer* server, int dbnum, char* key, size_t key_len) {
    ...
    if (client->db->filter_list.head) {
        ...
        while((node = next_filter_node(iter)) != NULL) {
            ...
            if (ret == REDIS_OK && client->returncode != REDIS_OK) {
                break;
            }
        }
