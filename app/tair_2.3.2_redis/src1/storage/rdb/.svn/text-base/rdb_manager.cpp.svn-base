...
int rdb_manager::lazyclear(int area, const char* password, const int len)
{
    redis_db *db = get_redis_instance();
    VERIFY_NAMESPACE(area);
    int ret = db->lazyflushdb(area, password, len);
    return redis_tair_code(ret);
}
...
int rdb_manager::setnsattr(int area, const char* attr, const int attr_len,
        const char* val, const int val_len, const char* password, const int len)
{
    redis_db *db = get_redis_instance();
    VERIFY_NAMESPACE(area);
    int ret = db->setnsattr(area, attr, attr_len, val, val_len, password, len);
    return redis_tair_code(ret);
}

int rdb_manager::getnsattr(int area, const char* attr, const int attr_len,
        char** val, int* val_len, const char* password, const int len)
{
    redis_db *db = get_redis_instance();
    VERIFY_NAMESPACE(area);
    int ret = db->getnsattr(area, attr, attr_len, val, val_len, password, len);
    return redis_tair_code(ret);
}
...
void rdb_manager::get_redis_config(redisConfig* config)
{
  ...
  config->password = TBSYS_CONFIG.getString(TAIRRDB_SECTION, RDB_AREA_PASSWORD, RDB_AREA_DEFAULT_PASSWORD);
