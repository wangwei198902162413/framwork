...
#define REDIS_IS_DUMPING                    7
#define REDIS_IS_LOADING                    6
...
#define REDIS_ERR_ATTR                      -25
#define REDIS_VERIFY_FAILED                 -26
...
/* status can't be binary code 110b or 111b, when it loading, it can't be dumping,
 * when it dumping, it can't be loading */
#define REDIS_DB_STATUS_WORKING     0
#define REDIS_DB_STATUS_FORBIDDEN   1
#define REDIS_DB_STATUS_LOADING     2
#define REDIS_DB_STATUS_DUMPING     4

typedef struct redisAreaConf {
    /* redis area */
    int id;
    /* redis Db structure limit */
    int list_max_size;
    int hash_max_size;
    int set_max_size;
    int zset_max_size;
    char* password;
    /* if you will access this structure, please lock this lock */
...
struct redisServer {
    ...
    redisAreaConf *area_confs;
	//有若干db，对应的就有若干锁
    redisDb *db;
    redisClient **clients;
	//redisServer有若干db，在多线程的交互下，需要加锁
...
redisClient *createClient(struct redisServer *server, int index, int area);
