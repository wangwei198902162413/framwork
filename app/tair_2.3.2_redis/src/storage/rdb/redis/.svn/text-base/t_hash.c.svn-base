...
#define CHECK_HASH_LENGTH(lobj) do {                            \
    ...
    if(hash_len + ((c->argc - 2) >> 1) > (unsigned long)(c->area_conf->hash_max_size)) {       \
        c->retvalue.llnum = 0;                                  \
...
void hsetCommand(redisClient *c) {
    ...
    if(update) {
        ...
        if (hash_len > (unsigned long)(c->area_conf->hash_max_size)) {
...
void filterCommand(redisClient *c) {
    ...
    /* use it to filter */
    if (hashTypeGet(o,c->argv[2],&value,&v,&vlen) == -1) {
        if (hashTypeLength(o) == 0) {
            dbDelete(c->db, c->argv[1]);
            c->returncode = REDIS_OK_NOT_EXIST;
        }
